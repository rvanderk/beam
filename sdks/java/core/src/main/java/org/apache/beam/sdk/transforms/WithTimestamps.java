 ...
 *
 * <p>Timestamps are used to assign {@link BoundedWindow Windows} to elements within the {@link
 * Window#into(org.apache.beam.sdk.transforms.windowing.WindowFn)} {@link PTransform}. Assigning
 * timestamps is useful when the input data set comes from a {@link Source} without implicit
 * timestamps (such as {@link org.apache.beam.sdk.io.TextIO.Read TextIO}).
 */
public class WithTimestamps<T> extends PTransform<PCollection<T>, PCollection<T>> {
  ...
  private static class AddTimestampsDoFn<T> extends DoFn<T, T> {
    ...
    public void processElement(ProcessContext c) {
      Instant timestamp = fn.apply(c.tenantAwareElement()).getValue();
      checkNotNull(
