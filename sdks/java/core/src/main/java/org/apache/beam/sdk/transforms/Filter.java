...
public class Filter<T> extends PTransform<PCollection<T>, PCollection<T>> {
  ...
  public static <T extends Comparable<T>> Filter<T> lessThan(final T value) {
    return by((SerializableFunction<T, Boolean>)
            input ->
                TenantAwareValue.of(input.getTenantId(), input.getValue().compareTo(value) < 0))
            ...
            ParDo.of(
                new DoFn<T, T>() {
                  ...
                  public void processElement(ProcessContext c) {
                    if (predicate.apply(c.tenantAwareElement()).getValue()) {
                      c.output(c.element());
                    }
                  }
                }))
