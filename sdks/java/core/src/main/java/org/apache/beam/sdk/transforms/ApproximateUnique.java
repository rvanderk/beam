...
public class ApproximateUnique {
  ...
  public static class ApproximateUniqueCombineFn<T>
    ...
    @Override
    public TenantAwareValue<LargestUnique> createAccumulator() {
      return TenantAwareValue.of(TenantAwareValue.NULL_TENANT, new LargestUnique(sampleSize));
    }

    @Override
    public TenantAwareValue<LargestUnique> addInput(
        TenantAwareValue<LargestUnique> tHeap, TenantAwareValue<T> input) {
      try {
        LargestUnique heap = tHeap.getValue();
        heap.add(hash(input.getValue(), coder));
        return TenantAwareValue.of(input.getTenantId(), heap);
      } catch (Throwable e) {
    ...
    @Override
    public TenantAwareValue<LargestUnique> mergeAccumulators(
        Iterable<TenantAwareValue<LargestUnique>> heaps) {
      Iterator<TenantAwareValue<LargestUnique>> iterator = heaps.iterator();
      TenantAwareValue<LargestUnique> firstHeap = iterator.next();
      LargestUnique heap = firstHeap.getValue();

      while (iterator.hasNext()) {
        List<Long> largestHashes = iterator.next().getValue().extractOrderedList();
      ...
      return TenantAwareValue.of(firstHeap.getTenantId(), heap);
    }

    @Override
    public TenantAwareValue<Long> extractOutput(TenantAwareValue<LargestUnique> heap) {
      List<Long> largestHashes = heap.getValue().extractOrderedList();
      if (largestHashes.size() < sampleSize) {
        return TenantAwareValue.of(heap.getTenantId(), (long) largestHashes.size());
      } else {
        ...
        double estimate =
            Math.log1p(-sampleSize / sampleSpaceSize)
                ...
                * HASH_SPACE_SIZE
                / sampleSpaceSize;
        return TenantAwareValue.of(heap.getTenantId(), Math.round(estimate));
      }
    }

    @Override
    public TenantAwareValueCoder<LargestUnique> getAccumulatorCoder(
        CoderRegistry registry, Coder<T> inputCoder) {
      return TenantAwareValueCoder.of(SerializableCoder.of(LargestUnique.class));
    }
