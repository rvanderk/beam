...
public class CountingSource {
  ...
  static class NowTimestampFn implements SerializableFunction<Long, Instant> {
    ...
    public TenantAwareValue<Instant> apply(TenantAwareValue<Long> input) {
      return TenantAwareValue.of(input.getTenantId(), Instant.now());
    }
  ...
  private static class UnboundedCountingReader extends UnboundedReader<Long> {
    ...
    public boolean advance() throws IOException {
      ...
      elementsRead.inc();
      current = nextValue;
      currentTimestamp = source.timestampFn.apply(TenantAwareValue.of("SYS0", current)).getValue();
      return true;
    }
    ...
    public Instant getWatermark() {
      return source.timestampFn.apply(TenantAwareValue.of("SYS0", current)).getValue();
    }
