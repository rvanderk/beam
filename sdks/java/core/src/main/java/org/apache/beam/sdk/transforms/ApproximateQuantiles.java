...
public class ApproximateQuantiles {
  ...
  public static class ApproximateQuantilesCombineFn<
    ...
    @Override
    public TenantAwareValue<QuantileState<T, ComparatorT>> createAccumulator() {
      return TenantAwareValue.of(
          TenantAwareValue.NULL_TENANT,
          QuantileState.empty(compareFn, numQuantiles, numBuffers, bufferSize));
    }

    @Override
    public TenantAwareValueCoder<QuantileState<T, ComparatorT>> getAccumulatorCoder(
        CoderRegistry registry, Coder<T> elementCoder) {
      return TenantAwareValueCoder.of(new QuantileStateCoder<>(compareFn, elementCoder));
    }
  ...
  static class QuantileState<T, ComparatorT extends Comparator<T> & Serializable>
    ...
    @Override
    public void addInput(TenantAwareValue<T> elem) {
      tenantId = elem.getTenantId();

      if (isEmpty()) {
        min = max = elem.getValue();
      } else if (compareFn.compare(elem.getValue(), min) < 0) {
        min = elem.getValue();
      } else if (compareFn.compare(elem.getValue(), max) > 0) {
        max = elem.getValue();
      }
      addUnbuffered(elem.getValue());
    }
     ...
     */
    @Override
    public TenantAwareValue<List<T>> extractOutput() {
      if (isEmpty()) {
        return TenantAwareValue.of(tenantId, Lists.newArrayList());
      }
      ...
      quantiles.add(0, min);
      quantiles.add(max);
      return TenantAwareValue.of(tenantId, quantiles);
    }
  }
