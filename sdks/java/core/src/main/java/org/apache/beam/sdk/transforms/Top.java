...
public class Top {
  ...
  public static class TopCombineFn<T, ComparatorT extends Comparator<T> & Serializable>
    ...
    @Override
    public TenantAwareValue<BoundedHeap<T, ComparatorT>> createAccumulator() {
      return TenantAwareValue.of(
          TenantAwareValue.NULL_TENANT, new BoundedHeap<>(count, compareFn, new ArrayList<>()));
    }

    @Override
    public TenantAwareValueCoder<BoundedHeap<T, ComparatorT>> getAccumulatorCoder(
        CoderRegistry registry, Coder<T> inputCoder) {
      return TenantAwareValueCoder.of(new BoundedHeapCoder<>(count, compareFn, inputCoder));
    }
  ...
  static class BoundedHeap<T, ComparatorT extends Comparator<T> & Serializable>
    ...
    @Override
    public void addInput(TenantAwareValue<T> value) {
      maybeAddInput(value);
    }
     ...
     */
    private boolean maybeAddInput(TenantAwareValue<T> value) {
      if (maximumSize == 0) {
        // Don't add anything.
        return false;
      }
      ...
      if (asQueue.size() < maximumSize) {
        asQueue.add(value.getValue());
        return true;
      } else if (compareFn.compare(value.getValue(), asQueue.peek()) > 0) {
        asQueue.poll();
        asQueue.add(value.getValue());
        return true;
      } else {
    ...
    public void mergeAccumulator(BoundedHeap<T, ComparatorT> accumulator) {
      for (T value : accumulator.asList()) {
        if (!maybeAddInput(TenantAwareValue.of(tenantId, value))) {
    ...
    public TenantAwareValue<List<T>> extractOutput() {
      return TenantAwareValue.of(tenantId, asList());
    }
