...
public class CombineWithContext {
  ...
  public abstract static class CombineFnWithContext<InputT, AccumT, OutputT>
     ...
     */
    public abstract TenantAwareValue<AccumT> createAccumulator(Context c);
     ...
     * {@code CombineWithContext.Context}.
     */
    public abstract TenantAwareValue<OutputT> extractOutput(
     ...
     */
    public TenantAwareValue<OutputT> apply(Iterable<TenantAwareValue<InputT>> inputs, Context c) {
      String tenantId = TenantAwareValue.NULL_TENANT;

      TenantAwareValue<AccumT> accum = createAccumulator(c);
      for (TenantAwareValue<InputT> input : inputs) {
        tenantId = input.getTenantId();
        accum = addInput(accum, input, c);
      }

      return extractOutput(TenantAwareValue.of(tenantId, accum.getValue()), c);
    }
