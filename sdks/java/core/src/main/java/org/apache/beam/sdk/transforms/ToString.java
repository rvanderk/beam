      ...
      return input.apply(
          MapElements.via(
              new SimpleFunction<Object, String>() {
                @Override
                public TenantAwareValue<String> apply(TenantAwareValue<Object> input) {
                  return TenantAwareValue.of(input.getTenantId(), input.toString());
                }
              }));
    }
  }
      ...
      return input.apply(
          MapElements.via(
              new SimpleFunction<KV<?, ?>, String>() {
                @Override
                public TenantAwareValue<String> apply(TenantAwareValue<KV<?, ?>> input) {
                  return TenantAwareValue.of(
                      input.getTenantId(),
                      input.getValue().getKey().toString()
                          + delimiter
                          + input.getValue().getValue().toString());
                }
              }));
    }
  }
      ...
      return input.apply(
          MapElements.via(
              new SimpleFunction<Iterable<?>, String>() {
                @Override
                public TenantAwareValue<String> apply(TenantAwareValue<Iterable<?>> input) {
                  return TenantAwareValue.of(
                      input.getTenantId(), Joiner.on(delimiter).join(input.getValue()));
                }
              }));
    }
  }
