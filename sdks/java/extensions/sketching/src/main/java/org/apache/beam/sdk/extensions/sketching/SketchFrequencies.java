 ...
 *
 * <ul>
 *   <li>{@code width = ceil(2 / epsilon)}
 *   <li>{@code depth = ceil(-log(1 - confidence) / log(2))}
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li>Use the {@link PTransform}s that return a {@link PCollection} singleton that contains a
 ...
 *
 * <p>One can tune the {@code epsilon} and {@code confidence} parameters in order to control
 ...
 *
 * <p>This example shows how to query the resulting {@link Sketch}. To estimate the number of hits
 ...
 * using the sketch as side input of a {@link ParDo}. <br>
 *
 * <pre><code>
 * {@literal PCollection<MyObject>} pc = ...;
 * {@literal PCollection<CountMinSketch>} countMinSketch = pc.apply(SketchFrequencies
 ...
 *
 * <p>This example is not really interesting but it shows how you can properly create a {@link
...
public final class SketchFrequencies {
   ...
   * PCollection<KV<K, V>>} and returns a {@code PCollection<KV<K, {@link CountMinSketch}>>}.
   *
  ...
  public abstract static class GlobalSketch<InputT>
    ...
    public PCollection<Sketch<InputT>> expand(PCollection<InputT> input) {
      return input.apply(
          ...
          Combine.<InputT, Sketch<InputT>>globally(
              CountMinSketchFn.<InputT>create(input.getCoder())
  ...
  public abstract static class PerKeySketch<K, V>
    ...
    public PCollection<KV<K, Sketch<V>>> expand(PCollection<KV<K, V>> input) {
      ...
      return input.apply(
          ...
          Combine.<K, V, Sketch<V>>perKey(
              CountMinSketchFn.<V>create(inputCoder.getValueCoder())
                  .withAccuracy(relativeError(), confidence())));
    }
  }
  ...
  public static class CountMinSketchFn<InputT>
    ...
    @Override
    public TenantAwareValue<Sketch<InputT>> createAccumulator() {
      return TenantAwareValue.of(
          TenantAwareValue.NULL_TENANT, Sketch.<InputT>create(epsilon, confidence));
    }

    @Override
    public TenantAwareValue<Sketch<InputT>> addInput(
        TenantAwareValue<Sketch<InputT>> accumulator, TenantAwareValue<InputT> element) {
      accumulator.getValue().add(element.getValue(), inputCoder);

      return TenantAwareValue.of(element.getTenantId(), accumulator.getValue());
    }

    @Override
    public TenantAwareValue<Sketch<InputT>> mergeAccumulators(
        Iterable<TenantAwareValue<Sketch<InputT>>> accumulators) {
      Iterator<TenantAwareValue<Sketch<InputT>>> it = accumulators.iterator();
      TenantAwareValue<Sketch<InputT>> first = it.next();
      CountMinSketch mergedSketches = first.getValue().sketch();
      try {
        while (it.hasNext()) {
          mergedSketches = CountMinSketch.merge(mergedSketches, it.next().getValue().sketch());
        }
      ...
      return TenantAwareValue.of(first.getTenantId(), Sketch.<InputT>create(mergedSketches));
    }

    /** Output the whole structure so it can be queried, reused or stored easily. */
    @Override
    public TenantAwareValue<Sketch<InputT>> extractOutput(
        TenantAwareValue<Sketch<InputT>> accumulator) {
      return accumulator;
    }

    @Override
    public TenantAwareValueCoder<Sketch<InputT>> getAccumulatorCoder(
        CoderRegistry registry, Coder inputCoder) {
      return TenantAwareValueCoder.of(new CountMinSketchCoder<>());
    }
